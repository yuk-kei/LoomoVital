...
public class PoseGraphic extends GraphicOverlay.Graphic {
    ...
    private static final float DESIRED_IN_FRAME_LIKELIHOOD = 0.9f;

    private final Pose pose;
    private final PoseProcessor poseProcessor;
...
//    private final List<String> poseClassification;
    ...
    private final Paint yelloPaint;
...
//            boolean visualizeZ,
        ...
        super(overlay);
        this.pose = pose;
        this.poseProcessor = new PoseProcessor(pose);

        this.showInFrameLikelihood = showInFrameLikelihood;
        this.visualizeZ = false;
...
//        this.poseClassification = poseClassification;
    ...
    public void draw(Canvas canvas) {
        ...
        List<PoseLandmark> landmarks =poseProcessor.landmarks;
        if (landmarks.isEmpty()) {
            return;
        }

        setPostureColor(canvas);

        // Draw all the points
        for (PoseLandmark landmark : poseProcessor.landmarks) {
...
//        }
    ...
    private void setPostureColor(Canvas canvas) {
        if(poseProcessor.isLying()){
            leftPaint.setColor(Color.RED);
            rightPaint.setColor(Color.RED);

            canvas.drawText(String.format(Locale.US, "Danger!"),300,200,redPaint);
        } else if (poseProcessor.isSeating()){
            leftPaint.setColor(Color.YELLOW);
            rightPaint.setColor(Color.YELLOW);

            canvas.drawText(String.format(Locale.US, "Warning!"),300,200,yelloPaint);
        } else if (poseProcessor.isStanding()){
            leftPaint.setColor(Color.GREEN);
            rightPaint.setColor(Color.GREEN);

            canvas.drawText(String.format(Locale.US, "Normal"),300,200,greenPaint);
        }else {
            leftPaint.setColor(Color.GREEN);
            rightPaint.setColor(Color.YELLOW);
        }
    ...
    void drawPoint(Canvas canvas, PoseLandmark landmark, Paint paint) {
        if(landmark.getInFrameLikelihood() < DESIRED_IN_FRAME_LIKELIHOOD){
            return;
        }
    ...
    void drawLine(Canvas canvas, PoseLandmark startLandmark, PoseLandmark endLandmark, Paint paint) {
        if(startLandmark.getInFrameLikelihood() < DESIRED_IN_FRAME_LIKELIHOOD
                || endLandmark.getInFrameLikelihood() < DESIRED_IN_FRAME_LIKELIHOOD){
            return;
        }
